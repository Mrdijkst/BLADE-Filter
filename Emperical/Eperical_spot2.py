...
def load_and_preprocess_data(file_path, price_area='DK2'):
    ...
    y = daily_prices.valuesdaily_prices = np.log(daily_prices)
    print(f"Daily averages: {len(daily_prices)} observations")
    print(f"Daily price statistics - Min: {daily_prices.min():.2f} DKK, Max: {daily_prices.max():.2f} DKK, Mean: {daily_prices.mean():.2f} DKK")
    ...
    # monday_prices = monday_df.groupby('Date')['SpotPriceDKK'].mean()
...
def fit_location_model(y, alpha_loss=None, **kwargs):
    ...
    **kwargs : dict
    ...
    try:
        ...
        model = RobustQLELocationModel(alpha_loss=alpha_loss, **kwargs)
