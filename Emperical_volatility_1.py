...
class RobustQLEVolatilityModel:
    ...
    def _qle_objective(self, params: np.ndarray, y: np.ndarray) -> float:
        ...
        try:
            ...
            if params[0] <= 0 or params[1] < 0 or params[2] < 0 or params[2] >= 1 or params[1]/params[4] + params[2] >= 0.999:
                return 1e10
    ...
    def plot_volatility(self, y: np.ndarray, gas_vol= None, true_vol: np.ndarray = None, title: str = "Estimated Volatility") -> None:
        ...
        # Plot original data
        plt.subplot(2, 1, 1)
        plt.plot(y[100:300], 'b-', alpha=0.5, label='Returns')
        plt.title(title)
        plt.legend()
        
        # Plot volatility
        plt.subplot(2, 1, 2)
        plt.plot(np.sqrt(self.fitted_volatility)[100:300], 'r-', label='Estimated Volatility')
        if gas_vol is not None:
            plt.plot(np.sqrt(gas_vol)[100:300], 'g-', label='GAS Volatility')
        plt.plot(np.abs(y)[100:300], alpha=0.3, label='Absolute Returns')
...
def main():
    ...
    plt.plot(data, 'b-', alpha=0.5, label='Returns')
    plt.show()
    ...
    # Plot the results
    model.plot_volatility(gas_vol=fitted_volatility, y=data.values)
