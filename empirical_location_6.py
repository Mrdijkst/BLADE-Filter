...
class RobustQLELocationModel:
    ...
    def fit(self, y: np.ndarray, initial_params: Optional[Dict] = None, 
        ...
        if initial_params is None:
            ...
            if self.alpha_loss is None:
                initial_params['alpha_loss'] = 1.6  # Default value close to Cauchy loss
    ...
    def plot_location(self, gas_est: None, date_index : None , y: np.ndarray, true_loc: np.ndarray = None, title: str = "Estimated Location") -> None:
        ...
        plt.plot(date_index[0:100], self.fitted_location[0:100], 'r-', linewidth=2, label='Estimated Location')
        # Plot de data zelf (y) tegen dezelfde index
        plt.plot(date_index[0:100], y[0:100], 'g--', alpha=0.8, label='Transformed Data')
        plt.plot(date_index[0:100], gas_est[0:100], 'b--', alpha=0.8, label='Gas Estimation')
        plt.title(title)
        plt.ylim(50,80)
        plt.legend()
        plt.tight_layout()
...
if __name__ == "__main__":
    ...
    model_gas =  GAS_t_Location_Model(y)
    model_gas.fit()
    if model is None:
    ...
    model.plot_location( model_gas.get_fitted_location()[0:300], monday_prices.index[0:300],  y[0:300], title="Estimated Weekly Location - Nord Pool Spot Price (Denmark DK2)")  
    plt.show()
