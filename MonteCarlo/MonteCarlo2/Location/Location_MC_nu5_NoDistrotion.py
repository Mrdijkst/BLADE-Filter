...
def simulate_location_dgp_train_only_heavytail(
    T_train: int,
    T_test: int,
    omega: float,
    alpha: float,
    ...
    """
    Upper-right DGP:
    - Train: symmetric mixture eps*t_nu + (1-eps)*t_nu_tilde
    - Test: clean t_nu
    - No standardization
    """
...
def run_one_simulation_upper_right(
    ...
    # --------------------------------------------------
    y_train, y_test = simulate_location_dgp_train_only_heavytail(
        T_train, T_test,
        omega0, alpha0, beta0,
        nu=nu,
        eps=eps,
...
if __name__ == "__main__":
    ...
    gamma1_grid = np.array([-1, -0.5, 0, 0.5, 1.0, 1.5, 2.0])  # example gamma1 values
    eps = 0.97  # one ν-panel value
