...
def simulate_location_dgp_train_only_positive_contamination(
    T_train: int,
    T_test: int,
    omega: float,
    alpha: float,
    ...
    """
    Panel D (lower-right):
    - Train: eps * t_nu + (1-eps) * (t_{nu_tilde} | > 0)
    - Test: clean t_nu
    - NO structural break (omega constant)
    - No standardization
    """
    ...
    # --- TRAIN innovations: one-sided contamination ---
    u = rng.random(T_train)
    eta_clean = rng.standard_t(nu, size=T_train)
    eta_pos   = draw_positive_student_t(nu_tilde, T_train, rng)
    eta_train = np.where(u < eps, eta_clean, eta_pos)
...
def run_one_simulation_upper_right(
    eps,
    gamma1_grid,
    T_train=2500,
    T_test=1000,
    ...
    # --------------------------------------------------
    # 2. Simulate contaminated data
    # --------------------------------------------------
    y_train, y_test = simulate_location_dgp_train_only_positive_contamination(
        T_train, T_test,
        omega=omega0,
        alpha=alpha0,
        beta=beta0,
    ...
    n_benchmarks = 1
    LossMat = np.zeros((len(gamma2_list), len(gamma1_grid) + n_benchmarks))
...
if __name__ == "__main__":
    ...
    gamma1_grid = np.array([-20,-10,-7,-5,-3,-1,-0.5,0])  # example gamma1 values
    eps = 0.95 # one ν-panel value

    LossMat = run_one_simulation_upper_right(
        eps=eps,
