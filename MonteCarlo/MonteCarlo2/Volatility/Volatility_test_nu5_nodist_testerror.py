...
def simulate_scale_dgp_train_only_heavytail(
    ...
    return y[:T_train], y[T_train:], sigma2_true_test

def simulate_scale_dgp_train_test_heavytail(
    T_train,
    T_test,
    omega,
    alpha,
    beta,
    nu,
    eps_train,   # P(clean) in TRAIN
    eps_test,    # P(clean) in TEST  <-- new
    seed
):
    rng = np.random.default_rng(seed)
    ...
    # --- TRAIN innovations: mixture ---
    u_tr = rng.random(T_train)
    eta_clean_tr = rng.standard_t(nu, size=T_train)
    eta_heavy_tr = rng.standard_t(nu_tilde, size=T_train)
    eta_train = np.where(u_tr < eps_train, eta_clean_tr, eta_heavy_tr)

    # --- TEST innovations: mixture (new) ---
    u_te = rng.random(T_test)
    eta_clean_te = rng.standard_t(nu, size=T_test)
    eta_heavy_te = rng.standard_t(nu_tilde, size=T_test)
    eta_test = np.where(u_te < eps_test, eta_clean_te, eta_heavy_te)
...
def run_one_simulation_scale_train_contaminated(
    ...
    y_train, y_test, sigma2_true_test = simulate_scale_dgp_train_only_heavytail(
        T_train=T_train,
        T_test=T_test,
        omega=omega0,
        alpha=alpha0,
...
if __name__ == "__main__":
    ...
    eps_train = 0.98  # mostly clean train
    eps_test  = 1   # now ALSO disturbed test (set to 1.0 for clean test)
