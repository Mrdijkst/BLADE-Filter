...
def run_single_simulation(rep_id, sample_size=2000):
    """
    Function to run a single Monte Carlo simulation for alpha_loss = -inf model only.
    ...
    # Define column names for alpha=-inf fixed model
    fixed_alpha_neginf_columns = ['rep_id', 'omega', 'gamma', 'beta', 'c', 'convergence', 'rmse', 'runtime']
    
    # ---------- Fixed Alpha=-inf Model ----------
    fixed_alpha_neginf_model = RobustQLEVolatilityModel(alpha_loss=float('-inf'))
    start_time_fixed_neginf = time.time()
    
    try:
        fixed_alpha_neginf_model.fit(y, method='Nelder-Mead', maxiter=1000, verbose=False)
        fixed_alpha_neginf_convergence = True
    except Exception:
        fixed_alpha_neginf_convergence = False
        
    fixed_alpha_neginf_runtime = time.time() - start_time_fixed_neginf
    
    # Calculate RMSE
    if fixed_alpha_neginf_convergence and fixed_alpha_neginf_model.fitted_volatility is not None:
        fixed_alpha_neginf_rmse = np.sqrt(np.mean((fixed_alpha_neginf_model.fitted_volatility - true_vol)**2))
    else:
        fixed_alpha_neginf_rmse = np.nan
    
    # Record results
    fixed_alpha_neginf_result = [rep_id+1]  # rep_id
    
    if fixed_alpha_neginf_convergence:
        for param in fixed_alpha_neginf_columns[1:-3]:  # Skip rep_id, convergence, rmse, runtime
            if param in fixed_alpha_neginf_model.params:
                fixed_alpha_neginf_result.append(fixed_alpha_neginf_model.params[param])
            else:
                fixed_alpha_neginf_result.append(np.nan)
    else:
        # If convergence failed, fill with NaNs
        fixed_alpha_neginf_result.extend([np.nan] * (len(fixed_alpha_neginf_columns) - 4))
        
    fixed_alpha_neginf_result.extend([fixed_alpha_neginf_convergence, fixed_alpha_neginf_rmse, fixed_alpha_neginf_runtime])
    
    return fixed_alpha_neginf_result


def run_parallel_monte_carlo_simulation(num_repetitions, sample_size, num_processes=None):
    """
    Run Monte Carlo simulation in parallel using multiple CPU cores for alpha=-inf model only
    ...
    --------
    pandas.DataFrame
        Results of all Monte Carlo repetitions for alpha=-inf model
    """
    ...
    print("Running simulations for alpha_loss = -inf model only")
    ...
    fixed_alpha_neginf_df = pd.DataFrame(results, columns=fixed_alpha_neginf_columns)
    ...
    avg_time_per_sim = fixed_alpha_neginf_df['runtime'].mean()
    ...
    convergence_rate = fixed_alpha_neginf_df['convergence'].mean() * 100
    print(f"Convergence rate: {convergence_rate:.1f}%")
    
    if convergence_rate > 0:
        converged_results = fixed_alpha_neginf_df[fixed_alpha_neginf_df['convergence'] == True]
...
if __name__ == "__main__":
    ...
    fixed_alpha_neginf_results = run_parallel_monte_carlo_simulation(
    ...
    output_filename = f'monte_carlo_alpha_neginf_results_parallel_{sample_size}_{num_repetitions}.csv'
    fixed_alpha_neginf_results.to_csv(output_filename, index=False)
    print(f"\nResults saved to: {output_filename}")
    
    # Display first few rows as verification
    print("\nFirst 5 rows of results:")
    print(fixed_alpha_neginf_results.head())