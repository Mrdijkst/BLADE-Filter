...
def run_single_simulation(rep_id, sample_size=2000):
    """
    Function to run a single Monte Carlo simulation for all three model types.
...
def run_parallel_monte_carlo_simulation(num_repetitions, sample_size, num_processes=None):
    ...
    avg_time_per_sim = sum(fixed_alpha0_df['runtime'] + fixed_alpha2_df['runtime'] + estimated_df['runtime']) / num_repetitions
...
if __name__ == "__main__":
    ...
    fixed_alpha0_results, fixed_alpha2_results, estimated_results = run_parallel_monte_carlo_simulation(
        num_repetitions=num_repetitions, 
        sample_size=sample_size,
        num_processes=num_processes
    )
    # Save results to CSV files (now with MAE included)
    fixed_alpha0_results.to_csv('monte_carlo_fixed_alpha0_results_parallel_4000_500till1000_with_mae.csv', index=False)
    fixed_alpha2_results.to_csv('monte_carlo_fixed_alpha2_results_parallel_4000_500till1000_with_mae.csv', index=False)
    estimated_results.to_csv('monte_carlo_estimated_alpha_results_parallel_4000_500till1000_with_mae.csv', index=False)
