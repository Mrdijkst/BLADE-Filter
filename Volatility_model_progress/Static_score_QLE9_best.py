...
def demo_simulated_data():
    ...
    # Simulate data
    T = 4000
    y, true_vol = sim_model.simulate(T, dist='n', df=5, seed=41)
    
    # Create a model for estimation
    # Option 1: Fix alpha to a known value
    model1 = RobustQLEVolatilityModel(alpha_loss=float('-inf'))  # Fix to Cauchy loss
    ...
    for key, val in params2.items():
        print(f"  {key}: {val:.4f}")
    print(np.mean(model1.fitted_volatility/true_vol))
    print(np.mean(model2.fitted_volatility/true_vol))
    # Plot results
