...
#note this one uses e(y,f) = f_t-y_t^2
...
class RobustQLEVolatilityModel:
    ...
    def _compute_derivatives(self, y: np.ndarray, f: np.ndarray, params: np.ndarray) -> np.ndarray:
        ...
        d2psi0_df = self._rho_second_derivative(e0, alpha_loss, c) * (+1)
        ...
        for t in range(1, T):
            e_t = -(y[t-1]**2 - f[t-1])
            psi_t = self._rho_derivative(e_t, alpha_loss, c) * (-1)  # -1 because e_t = y_t^2 - f_t
            d2psi_df = self._rho_second_derivative(e_t, alpha_loss, c) * (-1)**2  # Chain rule
