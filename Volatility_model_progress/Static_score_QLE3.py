...
class RobustQLEVolatilityModel:
    ...
    def _compute_derivatives(self, y: np.ndarray, f: np.ndarray, params: np.ndarray) -> np.ndarray:
        ...
        dpsi_dalpha1 =  grad(self._rho_derivative, argnum=1) 
        
        # According to the recursive formula:
        # ∂f_{t+1}(θ)/∂θ = ∂ω/∂θ + ψ_t*∂γ/∂θ + γ*∂ψ_t/∂θ + f_t*∂β/∂θ + (γ*∂ψ_t/∂f_t + β)*∂f_t/∂θ
        
        for t in range(1, T):
            e_t = (y[t-1]**2 - f[t-1])/(f[t-1]**2) 
            psi_t = self._rho_derivative(e_t, alpha_loss, self.c) * (-1)  # -1 because e_t = y_t^2 - f_t
            ...
            if self.alpha_loss is None:
                ...
                # dpsi_dalpha0 = (psi_plus - psi_minus) / (2 * delta)
                
                dpsi_dalpha2 = dpsi_dalpha1(e_t, alpha_loss, self.c)   

               
                
                # For alpha_loss (∂α/∂θ_3 = 1, else 0)
                df_dtheta[t, 3] = gamma * dpsi_dalpha2 + (beta+ d2psi_df*gamma) * df_dtheta[t-1, 3]
    ...
    def _qle_objective(self, params: np.ndarray, y: np.ndarray) -> float:
        ...
        try:
            ...
            # Form the QLE estimating equation (Equation 3 in the thesis)
    ...
    def fit(self, y: np.ndarray, initial_params: Optional[Dict] = None, 
        ...
        #     self._qle_objective,
        #     bounds=[(0.001, 0.5), (0.001, 0.5), (0.6, 0.999), (-5, 5)],  # bounds for [omega, gamma, beta, alpha_loss]
        #     args=(y,),
        #     maxiter=maxiter,
        # #)
...
def analyze_real_data(data: pd.Series):
    ...
    aligned_data = data.iloc[:len(model_adaptive.fitted_volatility)]

    rmse2 = np.sqrt(np.mean((model_adaptive.fitted_volatility - aligned_data**2)**2))
    ...
    print(f"RMSE of volatility estimation (Adaptive loss): {rmse2:.6f}")
...
if __name__ == "__main__":
    ...
    #model1 = grid_search_alpha(alpha_values=None)
    ...
    # models = analyze_real_data(log_returns)
